...
class TestTaskTypes(BaseTest):
    ...
    def setUpClass(cls) -> None:
        ...
        Group.objects.create(description="test", id="default", allowed_types=[FakeType.type_name])
    ...
    # region Creation
    def test_on_create_ok(self) -> None:
        ...
        ws = task_type.work_session_manager.work_session.objects.get(user=user, task=task)
    ...
    def test_on_create_twice(self) -> None:
        task_type = self.FAKE_TYPE
        user = User(username="user0", email="user0@email.com").save()
        task = task_type.task_model(
            id="task0",
        ...
        fake_datetime = datetime.now(timezone.utc) - timedelta(days=5)

        with patch("vulyk.ext.worksession.datetime") as mock_date:
            mock_date.now = lambda _: fake_datetime
            task_type.work_session_manager.start_work_session(task, user.id)

        ws_first = task_type.work_session_manager.work_session.objects.get(user=user, task=task)
        # re-create later
        task_type.work_session_manager.start_work_session(task, user.id)

        ws_second = task_type.work_session_manager.work_session.objects.get(user=user, task=task)
    ...
    def test_update_session_normal(self) -> None:
        task_type = self.FAKE_TYPE
        user = User(username="user0", email="user0@email.com").save()
        task = task_type.task_model(
            id="task0",
        ...
        duration = 50
        fake_datetime = datetime.now(timezone.utc) - timedelta(seconds=70)
    ...
    def test_update_session_twice(self) -> None:
        task_type = self.FAKE_TYPE
        user = User(username="user0", email="user0@email.com").save()
        task = task_type.task_model(
            id="task0",
        ...
        duration = 50
        fake_datetime = datetime.now(timezone.utc) - timedelta(seconds=110)
    ...
    def test_update_session_overdrive(self) -> None:
        task_type = self.FAKE_TYPE
        user = User(username="user0", email="user0@email.com").save()
        task = task_type.task_model(
            id="task0",
        ...
        duration = 50
        fake_datetime = datetime.now(timezone.utc) - timedelta(seconds=30)

        with patch("vulyk.ext.worksession.datetime") as mock_date:
            mock_date.now = lambda _: fake_datetime
            task_type.work_session_manager.start_work_session(task, user.id)

        self.assertRaises(WorkSessionUpdateError, lambda: task_type.record_activity(user.id, task.id, duration))

        session = WorkSession.objects.get(user=user.id, task=task)

        self.assertEqual(session.activity, 0)

    def test_update_session_negative(self) -> None:
        task_type = self.FAKE_TYPE
        user = User(username="user0", email="user0@email.com").save()
        task = task_type.task_model(
            id="task0",
    ...
    def test_update_session_not_found(self) -> None:
        ...
        self.assertRaises(TaskNotFoundError, lambda: fake_type.record_activity("fake_id", "", 0))

    # endregion Record activity

    # region On task done
    def test_on_done_ok(self) -> None:
        task_type = self.FAKE_TYPE
        user = User(username="user0", email="user0@email.com").save()
        task = task_type.task_model(
            id="task0",
        ...
        self.assertLess(ws.end_time.replace(tzinfo=timezone.utc) - datetime.now(timezone.utc), timedelta(seconds=1))

    # endregion On task done

    # region Stats
    def test_total_time_approximate(self) -> None:
        task_type = self.FAKE_TYPE
        user = User(username="user0", email="user0@email.com").save()
        tasks = [
        ...
        self.assertEqual(ws.get_total_user_time_approximate(user.id), 90)

    def test_total_time_precise(self) -> None:
        task_type = self.FAKE_TYPE
        user = User(username="user0", email="user0@email.com").save()
        tasks = [
            task_type.task_model(
        ...
        self.assertEqual(ws.get_total_user_time_precise(user.id), 150)
