    ...
    for blueprint in enabled_blueprints:
        try:
            ...
            if not isinstance(blueprint_obj, VulykModule):
                raise ImportError("Wrong blueprint type: {}".format(blueprint_obj))

            blueprint_obj.configure(blueprint.get("config", {}))

            app.register_blueprint(blueprint_obj, url_prefix="/{}".format(blueprint["url_prefix"]))

            app.logger.info("Blueprint {} loaded successfully.".format(blueprint["path"]))
        except (ImportError, KeyError) as e:
            app.logger.warning("Cannot load blueprint {}: {}.".format(blueprint.get("path"), e))
