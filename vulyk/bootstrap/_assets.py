...
def _get_files_for_settings(app: Flask, assets_key: str) -> list[str]:
    """
    ...
    if assets_key in app.config and len(app.config[assets_key]) > 0:
        result = [os.path.join(app.static_folder, static_file) for static_file in app.config[assets_key]]
...
def init(app: Flask) -> None:
    ...
    for key in ["js", "css"]:
        ...
        bundle = Bundle(
            *build_files, output=app.config["%s_OUTPUT" % assets_key], filters=app.config["%s_FILTERS" % assets_key]
        )
