...
def _init_plugin_assets(app: Flask, task_type: AbstractTaskType, static_path: PathLike | str) -> list[str]:
    """
    ...
    files_to_watch = []
    assets_location_map = {"js": task_type.JS_ASSETS, "css": task_type.CSS_ASSETS}

    for key in assets_location_map:
        ...
        if len(assets) > 0:
            ...
            output_path = "{folder}/{name}.{key}".format(folder=folder, name=name, key=key)
            bundle = Bundle(*files, output=output_path, filters=app.config.get(filters_name, ""))
            app.assets.register(name, bundle)
            app.logger.debug("Bundling files: %s%s", os.linesep, os.linesep.join(files))
...
def init_plugins(app: Flask) -> dict[str, AbstractTaskType]:
    """
    ...
    for plugin, task in enabled_tasks.items():
        ...
        task_settings = import_string("{plugin_name}.settings".format(plugin_name=plugin))
        plugin_type = import_string("{plugin_name}".format(plugin_name=plugin))
        settings = plugin_type.configure(task_settings)

        task_type = import_string("{plugin_name}.models.task_types.{task}".format(plugin_name=plugin, task=task))
        ...
        if "COLLECT_STATIC_ROOT" in app.config:
            ...
            static_path = os.path.join(app.static_folder, "{}{}".format(prefix, plugin))
        # else - use standard static folder
        else:
            static_path = default_static_path

        files_to_watch.extend(_init_plugin_assets(app=app, task_type=task_type, static_path=static_path))
